...
export const getStaffAdmins =
  (page = 1, limit = 10, query = "", adminId = "") =>
  (dispatch) => {
    dispatch(emptyError());
    let url = `staff/list?page=${page}&limit=${limit}&adminId=${adminId}`;

    if (query !== "" && query !== undefined)
      url = `staff/list?page=${page}&limit=${limit}&adminId=${adminId}&name=${
        query.name
      }&email=${encodeURIComponent(query.email)}&status=${
        query.status
      }&roleId=${query.roleId}`;
